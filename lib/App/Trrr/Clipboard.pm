...
sub clipboard {
    ...
    if ( $os eq 'ios' ) {
        ...
        if ( -e $pb_dir ) {
            opendir( my $pb_dir_dh, $pb_dir )
              || die "Can't opendir $pb_dir: $!";
            my ($pb_dir_dir) = grep { !/^\./ && !/^\.\.$/ && -d "$pb_dir/$_" }
              readdir($pb_dir_dh);
            closedir $pb_dir_dh;

            opendir( my $pb_dir_dir_dh, "$pb_dir/$pb_dir_dir" )
              || die "Can't opendir $pb_dir/$pb_dir_dir: $!";
            my @pb_dir_dir_file =
              grep { -f "$pb_dir/$pb_dir_dir/$_" } readdir($pb_dir_dir_dh);
